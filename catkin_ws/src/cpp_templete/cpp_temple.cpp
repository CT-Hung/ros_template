...
class hydrophone_data_node
{
    public:
        ...
        robotx_msgs::HydrophoneData hydro_msg;      // Hydrophone data message
...
{
    // Setup the publisher
    pub_sound = nh.advertise<robotx_msgs::HydrophoneData>("hydrophone_data", 10);
...
void hydrophone_data_node::run(void)
{
    ...
    {
        Capture();
        if(hydro_msg.data_ch1.size() >= ((int)pcm_sampleRate_ / 2)) // send data every 0.5 seconds
        {
